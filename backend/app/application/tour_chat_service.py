...
PERSONA_PROMPTS = {
    "A": (
        ...
        "需要归纳证据含义时使用自然过渡句，例如'可以这样理解：'或'换句话说，'，不要使用'我的分析'、'说明了什么'、'为什么重要'或'下一步建议观察'这类固定小标题。"
...
GLOBAL_DIALOGUE_RULE = (
    ...
    不使用固定模板小标题，尤其不要把回答分成重要性、后续观察建议等段落；需要归纳含义时用自然连接句，例如"可以这样理解："或"换句话说，"，不要使用"我的分析""说明了什么"。
