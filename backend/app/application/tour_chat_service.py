...
PERSONA_PROMPTS = {
    "A": (
        ...
        "需要归纳证据含义时使用'我的分析：'单独成行，再换行给出你的判断；不使用'说明了什么'、'为什么重要'或'下一步建议观察'这类固定小标题。"
    ...
    "B": (
        ...
        "当用户需要整理时，用'我的分析：'单独成行，再换行归纳观察现象，帮助用户理解疑惑点。"
    ...
    "C": (
        ...
        "需要总结时使用'我的分析：'单独成行，再换行解释具体材料和历史问题之间的关系。"
    ...
    "D": (
        ...
        "需要总结时使用'我的分析：'单独成行，再换行解释器物细节和半坡生活、技术或社会关系之间的联系。"
...
GLOBAL_DIALOGUE_RULE = (
    ...
    不使用固定模板小标题，尤其不要把回答分成重要性、后续观察建议等段落；需要归纳含义时统一使用"我的分析："单独成行，再换行输出你的总结，不要使用"说明了什么"。
