...
Rerank 会在生成回答前对检索结果进行二次排序，用于提升上下文相关性。该能力是可选功能。
...
| **类型** | 字符串 |
| **默认值** | `mock` |
| **可选值** | `siliconflow` / `openai` / `cohere` / `custom` / `mock` |

本地或测试环境如需禁用真实 Rerank 调用，请使用 `mock`。不要留空；空值或非法 provider 会在配置校验阶段直接报错。
...
OpenAI-compatible providers（`openai` / `cohere` / `custom`）必须配置该字段。当前 `siliconflow` provider 仍使用内置地址。
...
| **类型** | 字符串 |
| **默认值** | 空 |
| **要求** | 真实 provider（`siliconflow` / `openai` / `cohere` / `custom`）必须配置；`mock` 不需要 |
