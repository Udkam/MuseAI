...
当前处于 **上线准备与发布收口阶段**。后端已经支撑小程序的核心体验闭环，预想中的小程序功能已完成真机测试；正式上线仍受备案、微信合法域名、真实数据、OCR 服务、API key 治理和运维托管流程影响。完整执行手册见 [上线准备.md](../project_materials/docs/上线准备.md)。
...
- 已完成（服务器侧）：`api.banpo-museai.xyz` DNS、SSL 证书、Nginx 443 反代已配置，`https://api.banpo-museai.xyz/api/v1/health` 已返回 healthy。备案期间小程序可临时使用公网 HTTP 调试入口；正式上传前必须切回该 HTTPS 域名。
- 当前开发状态：备案期间小程序本地/开发者工具仍直接访问服务器 HTTP 调试入口 `http://122.152.232.190:3000/api/v1`；正式上传前必须切回 HTTPS。
- 未完成（微信侧）：备案、微信公众平台 request 合法域名配置、关闭开发者工具合法域名豁免后的正式环境真机联调。
...
- Nginx 反向代理到后端。
- 小程序备案期间当前可临时使用 `http://122.152.232.190:3000/api/v1` 调试；正式上传前必须切回 `https://api.banpo-museai.xyz/api/v1` 并关闭微信开发者工具合法域名豁免。
...
- 前端 API 已从开发 IP 切到 HTTPS 域名（待关闭开发者工具豁免后真机复验）。
