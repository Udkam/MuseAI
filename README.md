...
- 已完成（服务器侧）：`api.banpo-museai.xyz` DNS、SSL 证书、Nginx 443 反代已配置，`https://api.banpo-museai.xyz/api/v1/health` 已返回 healthy；前端 API 地址已统一切换到该 HTTPS 域名。
...
- Nginx 反向代理到后端。
- 小程序当前统一使用 `https://api.banpo-museai.xyz/api/v1`；微信正式环境仍受备案和合法域名审核限制。
