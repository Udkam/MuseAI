...
- 游览报告生成：到访展厅、认知变化、记录摘要、基础统计。
  - 到访展厅不只依赖 `hall_enter`，也会从提问、AI 回答、展品浏览和深挖事件中补齐。
  - 记录摘要基于用户问题与 AI 回答生成结构化笔记，不新增 LLM 调用。
...
报告统计依赖 `tour_events`，前端上报事件时应尽量使用 canonical hall slug。后端会继续兼容中文展厅名和历史 slug，并统一归一化为当前 slug。
...
这意味着用户只要在某个展厅至少提问一次，包括点击建议条触发提问，即使缺失 `hall_enter`，报告仍会把该展厅纳入 `halls_visited`。这样可以降低小程序页面切换、网络失败或历史缓存导致的统计漏记。
...
- `record_notes`：从用户问题和 AI 回答中抽取的记录摘要，供前端直接渲染。
